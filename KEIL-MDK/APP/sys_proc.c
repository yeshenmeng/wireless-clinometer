...
void sys_task_init(sys_task_t* task)
{
	...
	sys_obj.task.inclinometer_task = task->inclinometer_task;
...
void sys_task_schd(void)
{
	...
	if(CHECK_TASK_EVT(SYS_TASK_EVT_INCLINOMETER))
	{
		...
		sys_obj.task.inclinometer_task->task_start(); //启动倾角传感器采样任务
	}
...
/* 测斜仪任务停止事件 */
void inclinometer_task_stop_handler(void* param)
{
	sys_param_t* sys_param = sys_param_get_handle();
	/* 周期模式 */
	if(sys_param->iot_mode == 0)
	{
...
void lora_task_stop_handler(void* param)
{
	...
	{
		SET_TASK_EVT(SYS_TASK_EVT_INCLINOMETER); //连接网关成功设置倾角任务运行
	}
	/* LORA从连接态到连接态 */
...
#endif
	SET_TASK_EVT(SYS_TASK_EVT_INCLINOMETER); //启动倾角任务
}
